...
class treeNode():
	...
	def disp(self, ind = 1):   # 将树以文本形式展现
		print('  '*ind, self.name, ' ', self.count)
		for child in self.children.values():
			child.disp(ind + 1)

def loadDataSet():    # 简单数据集
	simpDat = [['r', 'z', 'h', 'j', 'p'],
	           ['z', 'y', 'x', 'w', 'v', 'u', 't', 's'],
	           ['z'],
	           ['r', 'x', 'n', 'o', 's'],
...
def createInitSet(dataset):     # 初始化数据集，统计项数量
	retDict = {}
...
# 		inTree.children[items[0]].inc(count)   # 更新 最大元素，对应的 treeNode 对象的count进行叠加
...
# 			headerTable[items[0]][1] = inTree.children[items[0]]  # headerTable只记录第一次节点出现的位置
# 		else:
...
def createTree(dataset, minSup=1):
	...
	for trans in dataset:    # 循环 dist{行：出现次数}的样本数据
		for item in trans:     # 对所有的行进行循环，得到行里面的所有元素, 统计每一行中每个元素出现的次数
			headerTable[item] = headerTable.get(item, 0) + dataset[trans]   # 例如： {'ababa': 3}  count(a)=3+3+3=9   count(b)=3+3=6
	for k in list(headerTable.keys()):  # 删除 headerTable中，元素次数<最小支持度的元素
		if headerTable[k] < minSup:
			del(headerTable[k])
	freqItemSet = set(headerTable.keys())   # 满足minSup: set(各元素集合)
	if len(freqItemSet) == 0:    # 频繁项集不存在，直接返回None
		return None, None
...
def findPrefixPath(basePat, treeNode):
	...
	"""
	condPats = {}
	while treeNode is not None:   #  对treeNode的link进行循环
		prefixPath = []
...
if __name__ == '__main__':
	...
	#print('初始化之后为：', initset)
	myFPTree, myHeaderTable = createTree(initset, 3)
	myFPTree.disp()
	# 抽取条件模式基
	# 查询树节点的，频繁子项
	print('x --->', findPrefixPath('x', myHeaderTable['x'][1]))
	print('z --->', findPrefixPath('z', myHeaderTable['z'][1]))
	print('r --->', findPrefixPath('r', myHeaderTable['r'][1]))
	# 创建条件模式基
	freqItemList = []
	mineTree(myFPTree, myHeaderTable, 3, set([]), freqItemList)
	print("freqItemList: \n", freqItemList)
'''
