...
def load_DataSet(filename):   # 导入csv文件
	dataset = []
	with open(filename, 'r') as fr:
		for line in fr.readlines():
			if not line:   # 对于空的行，不做处理
				continue
			lineArr = []
			for featrue in line.split(','):   # 切割之后得到，数组元素（列表元素）？？
				str_f = featrue.strip()   # strip()返回除字符串头尾指定的字符生成的新字符串
				if str_f.isdigit():    # 判断是否是数字
					lineArr.append(float(str_f))    # 将数据集的列转换成float形式
				else:
					lineArr.append(str_f)   # 添加分类标签
			dataset.append(lineArr)
	return dataset

def cross_validation_split(dataset, n_folds):     # 交叉验证数据集拆分，划分成n_folds份数据
	'''
	:param dataset: 原始数据集
	:param n_folds: 数据集拆分成n_folds份
	...
	fold_size = int(len(dataset) / n_folds)      #  查看每份交叉数据集的数据量
	for i in range(n_folds):
		...
		while len(fold) < fold_size:    # 这里不能用 if，if 只是在第一次判断时起作用，while 执行循环，直到条件不成立
			...
			index = randrange(len(dataset_copy))   # 返回数据集的一个随机项索引值
			# 将对应索引 index 的内容从 dataset_copy 中导出，并将该内容从 dataset_copy 中删除。
			# pop() 函数用于移除列表中的一个元素（默认最后一个元素），并且返回该元素的值。
			# fold.append(dataset_copy.pop(index))   # 无放回的方式，将已选择的索引值删除
			fold.append(dataset_copy[index])   # 有放回的方式
		dataset_split.append(fold)
	# 由dataset分割出的n_folds个数据构成的列表，为了用于交叉验证
	return dataset_split

def test_split(index, value, dataset):   # 根据特征和特征值分割数据集
...
def gini_index(groups, class_values):       # 计算分割数据集的基尼指数，分类越准确则基尼指数越小
	gini = 0.0
	for class_value in class_values:    # 类别取值
		for group in groups:    # 不同类别中，特征的分组值所属类别
			size = len(group)
			if size == 0:
				continue
			proportion = [row[-1] for row in group].count(class_value) / float(size)   # 计算特征分组值的所属类别的占比值
			gini += (proportion * (1.0 - proportion))  # 计算代价，分类越准确，则 gini 越小
	return gini

def get_split(dataset, n_features):    # 寻找分割数据集的最优特征index，特征值 row[index]，以及分割完的数据 groups（left, right）
	class_values = list(set(row[-1] for row in dataset))    # 取值为0、1
	b_index, b_value, b_score, b_groups = 999, 999, 999, None
	features = list()
	while len(features) < n_features:    #  随机选取不超过n_features个的特征进行数据拆分，选择出最优特征
	...
	for index in features:  # 在 n_features 个特征中选出最优的特征索引，并没有遍历所有特征，从而保证了每课决策树的差异性
		for row in dataset:
			groups = test_split(index, row[index], dataset)    #  分割数据
			gini = gini_index(groups, class_values)   #  计算gini指数
			if gini < b_score:     # 左右两边的数量越一样，说明数据区分度不高，gini系数越大
...
def split(node, max_depth, min_size, n_features, depth):     # 创建子分类器，递归分类，直到分类结束（随机森林是一棵二叉树）
	left, right = node['groups']
	del (node['groups'])
	if not left or not right:    # 检查不再分割的情况
...
def build_tree(train, max_depth, min_size, n_features):    # 创建一棵树，设定树的深度、叶子结点的大小、选取特征的个数
	'''
	:param train: 训练数据集
	:param max_depth: 决策树深度，深度越深，会导致过拟合
	:param min_size: 叶子结点的大小
...
def predict(node, row):   # 预测模型分类结果
	if float(row[node['index']]) < float(node['value']):
		if isinstance(node['left'], dict):    # isinstance用来判断一个对象是否为一个已知的类型
...
def bagging_predict(trees, row):   # bagging预测：参数trees树集合，row测试数据集的每一行记录；返回随机森林中决策树结果最大的
	'''
	:param trees: 决策树集合
	:param row: 测试数据集中的每一行数据
	:return: 返回随机森林中，决策树结果出现最大的部分
	'''
	predictions = [predict(tree, row) for tree in trees]  # 使用多个决策树trees对测试集test的第row行进行预测，再使用简单投票法判断出该行所属分类
	return max(set(predictions), key = predictions.count)

def subsample(dataset, ratio):    # 创建数据集的随机子样。参数：dataset训练数据集,ratio训练数据集子样比例；返回随机抽样的结果
	'''
	:param dataset: 训练数据集
	:param ratio: 训练数据集的样本比例
	:return: sample: 随机抽样的训练样本
	'''
	sample = list()
	# 训练样本的按比例抽样。
...
def random_forest(train, test, max_depth, min_size, sample_size, n_trees, n_features):
	# 评估算法性能，参数：训练集、测试集、决策树深度、叶节点大小、训练数据集的样本比例、决策树个数、选取特征个数；返回每一行的预测结果
	trees = list()
	for i in range(n_trees):
		sample = subsample(train, sample_size)    # 随机抽样的训练样本， 随机采样保证了每棵决策树训练集的差异性
		tree = build_tree(sample, max_depth, min_size, n_features)    # 根据抽取的数据集，创建树结构
		trees.append(tree)
...
def evaluate_algorithm(dataset, algorithm, n_folds, *args):   # 评估算法性能，返回模型得分
	# 参数：原始数据集、使用的算法、数据份数、其他参数；返回模型得分
	folds = cross_validation_split(dataset, n_folds)   # 将数据集抽样n_folds份，数据可以重复抽样，每一次的List中无重复
	score = list()
	for fold in folds:   # 每次循环从folds抽取一个fold作为测试集，其余作为训练集
		trainset = list(folds)
		trainset.remove(fold)
		trainset = sum(trainset, [])   # 将多个fold列表组合成一个trainset列表，类似于union all
		testset = list()
		for row in fold:    # fold 表示从原始数据集 dataset 提取出来的测试集
			row_copy = list(row)
			row_copy[-1] = None    # 测试数据集中新增一列
			testset.append(row_copy)
		predicted = algorithm(trainset, testset, *args)
		actual = [row[-1] for row in fold]
		accuracy = accuracy_metric(actual, predicted)   # 计算随机森林算法预测结果的准确率
		score.append(accuracy)
	return score

if __name__ == '__main__':
	dataset = load_DataSet(r'D:\Data\ML_InAction\集成算法\sonar_all_data.txt')
	#print(dataset)
	n_folds = 5           # 分5份数据，进行交叉验证
	max_depth = 10
	min_size = 1         # 决策树叶节点最少的元素数量
	sample_size = 1.0      # 做决策树时的样本比例
	#n_features = int((len(dataset[0]) - 1))
	n_features = 10    # 特征自选，可在模型的准确性和多样性相互选择
	n_trees = 1
	for n_trees in [1, 10, 20]:   # 理论上树的数量越多越好
		time1 = time.time()
		scores = evaluate_algorithm(dataset, random_forest, n_folds, max_depth, min_size, sample_size, n_trees, n_features)
		time2 = time.time()
		print('-*' * 25,'Trees: %d' % n_trees,'*-' * 25)
		print('Time Used: %s' % (time2 - time1))
		seed(1)
		print('random = ', random())
		print('Trees: %d' % n_trees)
		print('Scores: %s' % scores)
		print('Mean Accuracy: %.3f%%' % (sum(scores) / float(len(scores))))
	print('-*' * 25, 'Mission Completed', '*-' * 25)
...
#
#     # 加载数据
#     dataset = loadDataSet(r'D:\Data\ML_InAction\集成算法\sonar_all_data.txt')
#     # print(dataset)
#
